...
public class BlackJackGameListActivity extends ActionBarActivity implements IDialogListener {
				...
				for (int i=0; i<jsa.length(); i++) {
					...
					btnGame.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							Dialogs.showTwoButtonsDialog(BlackJackGameListActivity.this, "OK", "Do you wanna join "  + btnGame.getText() + "?", "Yes", "No");
						}
					});
		...
		if (button==IDialogListener.YES) {
			...
			try {
				...
				if (jso.getType().equals(OKMessage.class.getSimpleName())) {
					...
					if (store.getIdGame()== BlackJack.BJ) {
						Intent i=new Intent(this, BlackJackActivity.class);
						startActivity(i);
					} else {
